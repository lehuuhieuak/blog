...
export async function GET() {
  ...
  const staticURLs = [routes.home, routes.about]
  const entries = [...staticURLs.map((path) => `<url><loc>${escapeXML(canonicalURL(path))}</loc></url>`), ...data.map((article) => `<url><loc>${escapeXML(canonicalURL(routes.article(article.slug)))}</loc><lastmod>${new Date(article.updated_at).toISOString()}</lastmod></url>`)].join("")
