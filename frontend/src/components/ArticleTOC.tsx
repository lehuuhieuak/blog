...
export default function ArticleTOC({ items, collapsible = false }: Props) {
  if (collapsible) return <details className="toc toc--compact"><summary>Mục lục nội dung</summary><nav aria-label="Mục lục bài viết"><TOCItems items={items} /></nav></details>
  return <nav className="toc toc--desktop" aria-label="Mục lục bài viết"><p className="toc__title">Mục lục nội dung</p><TOCItems items={items} /></nav>
}
